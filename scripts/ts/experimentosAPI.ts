...
const enderecoAPI: Promise<Response> = fetch("http://localhost:3000/videos");
...
const obtendoVideos: Promise<void> = fetch("http://localhost:3000/videos").then((resposta) => {
  resposta.json().then((videos: Video[]) => console.log("teste:", videos));
});

const obtendoVideos2 = fetch("http://localhost:3000/videos").then((resposta) => {
  try {
    resposta.json().then((videos: Video[]) => console.log("teste2:", videos));
  } catch (error) {
